...
util.getMatchedLines = function (text, expression) {
    ...
    while (reg = regex.exec(text)) {
        ...
        if (lineContent) {
            preResult[line] = preResult[line] ||  {};
            preResult[line].matches = preResult[line].matches || [];

            preResult[line].line = lineContent.trim();
            preResult[line].matches.push(reg[0]);
            totalMatches += reg.length;
        }
    }

    for (var l in preResult) {
        var indexesInfo = getIndexes(preResult[l].matches, preResult[l].line);
        preResult[l].newLine = highLightMatch(preResult[l].line, indexesInfo);
        result.push(chalk.magenta(' ' + l +  ': ') + preResult[l].newLine);
    }

    return [result, totalMatches];

    function getIndexes(matches, line) {
        var result = [];
        var uniqMatches = util.uniq(preResult[l].matches);
        uniqMatches.forEach(function(el) {
            var regex = new RegExp(el, 'g');
            while (res = regex.exec(preResult[l].line)) {
